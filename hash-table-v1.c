...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	uint32_t error = pthread_mutex_init(&hash_table->mutex, NULL);
	if (error != 0)
	{
		exit(error);
	}
							 ...
							 uint32_t value)
{
	...
	// Lock the mutex
	uint32_t error = pthread_mutex_lock(&hash_table->mutex);
	if (error != 0)
	{
		exit(error);
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	// Destroy mutex / lock as specified in the spec
	uint32_t error = pthread_mutex_destroy(&hash_table->mutex);
	if (error != 0)
	{
		exit(error);
	}
